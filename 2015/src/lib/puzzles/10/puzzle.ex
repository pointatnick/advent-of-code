# Puzzle input: 1321131112
# Really slow starting at around the 28th iteration
...
defmodule Day10 do
  defmodule Puzzle do
    ...
    @spec decay(atom(), integer()) :: String.t()
    def decay(element, times) do
      if times == 0 do
        Map.get(@elements, element) |> elem(0)
      else
        Map.get(@elements, element) |> elem(1) |> Enum.map(&decay(&1, times - 1)) |> Enum.join()
      end
    end

    def solve(element, times) do
      decay(element, times) |> String.length()
    end
  end
end
